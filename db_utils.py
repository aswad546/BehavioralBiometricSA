...
# Adjust minconn and maxconn as appropriate for your workload.
try:
...
def create_multicore_static_info_table(drop_table: bool):
    """
    Create the multicore_static_info table.
    If drop_table is True, drop the table first.
    """
    create_table_query = ""
    if drop_table:
        create_table_query = '''
    ...
    try:
        with get_db_connection_context() as conn:
            ...
            conn.commit()
            cur.close()
        print("Table multicore_static_info created successfully (drop_table={}).".format(drop_table))
    except Exception as e:
        print("Error creating multicore_static_info table:", e)
...
def get_db_connection_context():
    ...
    ensures it is released back to the pool after use.
    """
...
def export_table_to_csv(argument):
    """Export the multicore_static_info table to CSV and truncate script_flow."""
    with get_db_connection_context() as conn:
        try:
            ...
            print(f"Exported multicore_static_info table to {output_path}")

            truncate_query = "TRUNCATE TABLE script_flow;"
            with conn.cursor() as cur:
                cur.execute(truncate_query)
                conn.commit()
            print("Truncated table script_flow.")
        except Exception as e:
            print("Error exporting table:", e)

def insert_into_table(table_name, data):
    """Prepare an INSERT statement with JSON conversion for dicts/lists."""
    columns = data.keys()
    values = []
...
def write_batch_to_database(batch):
    """Write a batch of inserts to the database."""
    with get_db_connection_context() as conn:
        cur = conn.cursor()
        try:
